...
/**
 * POST /api/hosts/[id]/containers/[containerId]/kill - Kill a container (SIGKILL)
 */
export async function POST(
  ...
  try {
    ...
    const result = await connector.killContainer(containerId);
    if (!result.success) {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to kill container" },
      { status: 500 }
    );
  }
}
